//===- memtile_row_store_compute_buffered.mlir -----------------*- MLIR -*-===//
//
...
// CHECK: %[[SRC0:.*]] = aie.buffer(%{{.*}}) {sym_name = "row_store0_src_0"} : memref<32x96xbf16>
// CHECK: %[[SRC1:.*]] = aie.buffer(%{{.*}}) {sym_name = "row_store0_src_1"} : memref<32x96xbf16>
// CHECK: %[[DST0:.*]] = aie.buffer(%{{.*}}) {sym_name = "row_store0_dst_0"} : memref<32x96xbf16>
// CHECK: %[[DST1:.*]] = aie.buffer(%{{.*}}) {sym_name = "row_store0_dst_1"} : memref<32x96xbf16>
...
// CHECK: aie.dma_bd(%[[SRC0]] : memref<32x96xbf16>, 0, 3072) {bd_id = 0 : i32, next_bd_id = 1 : i32}
// CHECK: aie.dma_bd(%[[SRC1]] : memref<32x96xbf16>, 0, 3072) {bd_id = 1 : i32, next_bd_id = 0 : i32}
// CHECK: aie.dma_bd(%[[DST0]] : memref<32x96xbf16>, 0, 3072) {bd_id = 2 : i32, next_bd_id = 3 : i32}
// CHECK: aie.dma_bd(%[[DST1]] : memref<32x96xbf16>, 0, 3072) {bd_id = 3 : i32, next_bd_id = 2 : i32}
