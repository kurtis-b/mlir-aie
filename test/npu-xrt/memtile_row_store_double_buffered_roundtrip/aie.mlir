...
module {
  aie.device(NPUDEVICE) {
    ...
    %input = aie.buffer(%tile_0_2) {sym_name = "input"} : memref<1xi32>
    %output = aie.buffer(%tile_0_2) {sym_name = "output"} : memref<1xi32>
    ...
    ^input_bd:  // 2 preds: ^bb0, ^input_bd
      ...
      aie.dma_bd(%input : memref<1xi32>, 0, 1)
    ...
    ^output_bd:  // 2 preds: ^out_dma, ^output_bd
      ...
      aie.dma_bd(%output : memref<1xi32>, 0, 1)
    ...
    aie.core(%tile_0_2) {
      ...
      affine.for %part = 0 to 4 {
        aie.use_lock(%input_full, AcquireGreaterEqual, 1)
        %src = aie.memtile_row_store.acquire @row_store(Produce) : memref<1xi32>
        %v = memref.load %input[%c0] : memref<1xi32>
        %w = arith.addi %v, %c100 : i32
        memref.store %w, %src[%c0] : memref<1xi32>
        aie.memtile_row_store.release @row_store(Produce)
        aie.use_lock(%input_empty, Release, 1)
      }

      affine.for %part = 0 to 4 {
        %dst = aie.memtile_row_store.acquire @row_store(Consume) : memref<1xi32>
        aie.use_lock(%output_empty, AcquireGreaterEqual, 1)
        aie.use_lock(%input_full, AcquireGreaterEqual, 1)
        %src = aie.memtile_row_store.acquire @row_store(Produce) : memref<1xi32>
        %v0 = memref.load %dst[%c0] : memref<1xi32>
        %w0 = arith.addi %v0, %c1_i32 : i32
        memref.store %w0, %output[%c0] : memref<1xi32>
        %v1 = memref.load %input[%c0] : memref<1xi32>
        %w1 = arith.addi %v1, %c100 : i32
        memref.store %w1, %src[%c0] : memref<1xi32>
        aie.memtile_row_store.release @row_store(Produce)
        aie.use_lock(%input_empty, Release, 1)
      ...
      affine.for %part = 0 to 4 {
        %dst = aie.memtile_row_store.acquire @row_store(Consume) : memref<1xi32>
        aie.use_lock(%output_empty, AcquireGreaterEqual, 1)
        %v = memref.load %dst[%c0] : memref<1xi32>
        %w = arith.addi %v, %c1_i32 : i32
        memref.store %w, %output[%c0] : memref<1xi32>
        aie.use_lock(%output_full, Release, 1)
        aie.memtile_row_store.release @row_store(Consume)
      }
      aie.end
