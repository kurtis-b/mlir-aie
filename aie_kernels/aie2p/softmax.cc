  ...
  for (int i = 0; i < M; i++)
    chess_prepare_for_pipelining chess_loop_range(1, ) {
      ...
      auto it_exp_out = aie::begin_vector<16>((bfloat16 *)output_vector +
                                              i * row_vector_size);
      auto it_scale = aie::cbegin_restrict_vector<16>(
          (bfloat16 *)output_vector + i * row_vector_size);
      auto it_soft_out = aie::begin_restrict_vector<16>(
          (bfloat16 *)output_vector + i * row_vector_size);

      bfloat16 col_sum_inv;
      aie::vector<bfloat16, 16> in_elems, va;
      aie::accum<accfloat, 16> out_vals;
      int col_iters = row_vector_size >> 4;
      accum_exp_val = 0;
