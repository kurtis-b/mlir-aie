...
void softmax_2d_bf16(bfloat16 *restrict input_vector,
                     bfloat16 *restrict output_vector) {
  event0();

  int row_vector_size = N;
  for (int i = 0; i < M; i++)
    chess_prepare_for_pipelining chess_loop_range(1, ) {
      ...
      auto it_log_in = aie::cbegin_restrict_vector<SM_VEC_LEN>((bfloat16 *)input_vector + i * row_vector_size);
      auto it_log_out = aie::begin_restrict_vector<SM_VEC_LEN>((bfloat16 *)input_vector + i * row_vector_size);
      auto it_exp_in = aie::cbegin_restrict_vector<SM_VEC_LEN>((bfloat16 *)input_vector + i * row_vector_size);
      auto it_exp_out = aie::begin_restrict_vector<SM_VEC_LEN>((bfloat16 *)output_vector + i * row_vector_size);
      auto it_scale = aie::cbegin_restrict_vector<SM_VEC_LEN>((bfloat16 *)output_vector + i * row_vector_size);
      auto it_soft_out = aie::begin_restrict_vector<SM_VEC_LEN>((bfloat16 *)output_vector + i * row_vector_size);
      ...
      float running_max = 0;
      bfloat16 col_sum_inv;
      const int elem_iters = row_vector_size / SM_VEC_LEN;
...
void softmax_bf16(bfloat16 *restrict input, bfloat16 *restrict output) {
  softmax_2d_bf16<DIM_M, DIM_N>(input, output);
}
