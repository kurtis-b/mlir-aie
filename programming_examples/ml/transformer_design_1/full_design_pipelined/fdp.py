...
def my_fdp(
    ...
    attn_score_mm_dims = (64, 64, 64)
    softmax_dims = (attn_score_mm_dims[0], attn_score_mm_dims[2])
    attn_score_v_mm_dims = (softmax_dims[0], softmax_dims[1], 64)
    output_mm_dims = (attn_score_v_mm_dims[0], attn_score_v_mm_dims[2], 64)
    fdp_dims = [attn_score_mm_dims, attn_score_v_mm_dims, output_mm_dims]
    ...
    for dims in proj_dims + fdp_dims:
        assert (
            M % dims[0]  == 0
    ...
    def device_body():
        ...
        #     f"zero_{dtype_out_str}_{attn_score_v_mm_dims[0]}_{attn_score_v_mm_dims[1]}_{attn_score_v_mm_dims[2]}_{row_major}", 
        ...
        #     f"zero_{dtype_out_str}_{output_mm_dims[0]}_{output_mm_dims[1]}_{output_mm_dims[2]}_{row_major}",
        #     inputs=[output_l1_ty]
        # )
        ...
        for row, l1_pos in enumerate(right_mtx_in[WQ_STR][L1_POS_STR]):
            @core(
                core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]],
                f"fdp_mm_{q_proj_dims[0]}x{q_proj_dims[1]}x{q_proj_dims[2]}_row_major.o",
                stack_size=0xF00
            )
            def core_body():
        ...
        for row, l1_pos in enumerate(right_mtx_in[WK_STR][L1_POS_STR]):
            @core(
                core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]],
                f"fdp_mm_{k_proj_dims[0]}x{k_proj_dims[1]}x{k_proj_dims[2]}_row_major.o",
                stack_size=0xF00
            )
            def core_body():
                for _ in range_(0xFFFFFFFF):
        ...
        for row, l1_pos in enumerate(right_mtx_in[WV_STR][L1_POS_STR]):
            @core(
                core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]],
                f"fdp_mm_{v_proj_dims[0]}x{v_proj_dims[1]}x{v_proj_dims[2]}_row_major.o",
                stack_size=0xF00
            )
            def core_body():
                for _ in range_(0xFFFFFFFF):
        ...
        for head, l1_pos in enumerate(left_mtx_in[Q_STR][L1_POS_STR]):
            @core(core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]], f"fdp_mm_{attn_score_mm_dims[0]}x{attn_score_mm_dims[1]}x{attn_score_mm_dims[2]}_col_major.o", stack_size=0xF00)
            def core_body():
                for _ in range_(0xFFFFFFFF):
        ...
        for head, l1_pos in enumerate(l1_fuse_mtx_in[ATTN_SCORE_STR][L1_POS_STR]):
            @core(core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]], f"fdp_softmax.o", stack_size=0xF00) # Make sure to use the bundled obj file, not the softmax-only obj file
            def core_body():
                for _ in range_(0xFFFFFFFF):
        ...
        for head, l1_pos in enumerate(l1_fuse_mtx_in[SOFTMAX_STR][L1_POS_STR]):
            @core(core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]], f"fdp_mm_{attn_score_v_mm_dims[0]}x{attn_score_v_mm_dims[1]}x{attn_score_v_mm_dims[2]}_row_major.o", stack_size=0xF00)
            def core_body():
                for _ in range_(0xFFFFFFFF):
                    for _ in range_(H // len(l1_fuse_mtx_in[SOFTMAX_STR][L1_POS_STR])):
                        ...
                        for _ in range_(head_dim // attn_score_v_mm_dims[2]):
                            ...
                            matmul_projs(elem_in_softmax, elem_in_v, elem_attn_score_v)
        ...
        for head, l1_pos in enumerate(l1_fuse_mtx_in[ATTN_SCORE_V_STR][L1_POS_STR]):
            @core(core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]], f"fdp_mm_{output_mm_dims[0]}x{output_mm_dims[1]}x{output_mm_dims[2]}_row_major.o", stack_size=0xF00)
            def core_body():
                for _ in range_(0xFFFFFFFF):
                    elem_output = output_l1l1_fifos[head].acquire(ObjectFifoPort.Produce, 1)
                    # zero_output(elem_output)
                    zero_projs(elem_output)
                    for _ in range_(K // output_mm_dims[1] // len(l1_fuse_mtx_in[ATTN_SCORE_V_STR][L1_POS_STR])):
                        ...
                        matmul_projs(elem_in_o3, elem_in_wo, elem_output)
        ...
        @core(core_tiles[l1_fuse_mtx_in[ACCUM_STR][L1_POS_STR][0][ROW_IDX]][l1_fuse_mtx_in[ACCUM_STR][L1_POS_STR][0][COL_IDX]], f"fdp_add_{output_mm_dims[0]}x{output_mm_dims[2]}.o", stack_size=0xF00)
        def core_body():
            for _ in range_(0xFFFFFFFF):
