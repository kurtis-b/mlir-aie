...
float abs_tol = ffn_1_common::get_abs_tol<C_DATATYPE>();
float rel_tol = ffn_1_common::get_rel_tol<C_DATATYPE>();

int main(int argc, const char *argv[]) {
  // Program arguments parsing
  cxxopts::Options options("Matrix Matrix Multiplication Test");
  cxxopts::ParseResult vm;
  ffn_1_common::add_default_options(options);

  ffn_1_common::parse_options(argc, argv, options, vm);
  ...
  for (int i = 0; i < A_VOLUME; i++) {
    AVec[i] = ffn_1_common::get_random<A_DATATYPE>();
  }
  memcpy(bufA, AVec.data(), (AVec.size() * sizeof(A_DATATYPE)));
  B_DATATYPE *bufB = bo_b.map<B_DATATYPE *>();
  std::vector<B_DATATYPE> BVec(B_VOLUME);
  for (int i = 0; i < B_VOLUME; i++) {
    BVec[i] = ffn_1_common::get_random<B_DATATYPE>() * i;
    // Diagonal:
    // if(i % N == i / N) {
    //   BVec[i] = 1.0;
    // } else {
  ...
  if (verbosity >= 2) {
    ...
    std::cout << "Verification tolerance " << abs_tol << " absolute, "
              << rel_tol << " relative.\n";
    std::cout << "A = \n";
    ffn_1_common::print_matrix(AVec, K);
    std::cout << "B = \n";
    ffn_1_common::print_matrix(BVec, N);
  }
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    if (do_verify) {
      ...
      if (do_verify_stochastic) {
        errors = ffn_1_common::verify_stochastic<A_DATATYPE, C_DATATYPE,
                                                  ACC_DATATYPE>(
            M, N, K, AVec, BVec, CVec, verify_stochastic_n_samples, verbosity,
            abs_tol, rel_tol, b_col_maj);
      } else {
        errors = ffn_1_common::verify<A_DATATYPE, C_DATATYPE, ACC_DATATYPE>(
  ...
  if (trace_size > 0) {
    ffn_1_common::write_out_trace((char *)bufTrace, trace_size,
