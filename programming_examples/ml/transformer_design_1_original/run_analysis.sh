...
for dir in "$BASEDIR/mha" "$BASEDIR/mha_by_steps/only_attn_steps" "$BASEDIR/mha_by_steps/only_proj_steps" "$BASEDIR/ffn-1" "$BASEDIR/ffn-2" "$BASEDIR/add_and_norm"; do
    if [ -d "$dir" ]; then
        ...
        python "$BASEDIR/scripts/run_analysis.py" --dev "$devicename" --input_file "$BASEDIR/$dirname/build/aie_${dirname}.mlir.prj/input_physical.mlir" --task mem-util --output_dir "$run_output_dir" >> "$BASEDIR/$results_dir/run_analysis.txt" 2>&1
        python "$BASEDIR/scripts/run_analysis.py" --dev "$devicename" --input_file "$BASEDIR/$dirname/build/aie_${dirname}.mlir" --task comp-dist --output_dir "$run_output_dir" >> "$BASEDIR/$results_dir/run_analysis.txt" 2>&1
        python "$BASEDIR/scripts/run_analysis.py" --dev "$devicename" --input_file "$BASEDIR/$dirname/build/aie_${dirname}.mlir" --task loop-iters --output_dir "$run_output_dir" >> "$BASEDIR/$results_dir/run_analysis.txt" 2>&1
        cd "$BASEDIR"
    fi
done
