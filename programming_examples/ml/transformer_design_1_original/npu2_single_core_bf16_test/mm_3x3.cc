...
template <typename T_in, typename T_out, unsigned rowA, unsigned colA,
          ...
          bool b_row_maj = true>
static inline void matmul_vectorized_3x3_mmul(const T_in *__restrict pA,
  ...
  for (unsigned z = 0; z < rowA; z += 3) {
    T_out *__restrict pC1 = pC + (z * colB) * MMUL::size_C;
    T_out *__restrict pC2 = pC + ((z + 1) * colB) * MMUL::size_C;
    T_out *__restrict pC3 = pC + ((z + 2) * colB) * MMUL::size_C;

    for (unsigned j = 0; j < colB; j += 3)
...
#endif
      {
        ...
        const T_in *__restrict pA3 = pA + ((z + 2) * colA) * MMUL::size_A;
...
matmul_vectorized_8x8x8_bf16_bf16(const bfloat16 *__restrict pA,
  ...
  static_assert(m % (3 * r) == 0);
  static_assert(k % s == 0);
  static_assert(n % (3 * t) == 0);

  return matmul_vectorized_3x3_mmul<bfloat16, bfloat16, (m / r), (k / s),
                                    (n / t), r, s, t, is_b_row_maj>(pA, pB, pC);
}
