...
void matmul(int M, int N, int K, int H, const std::vector<Tin> A,
  ...
  //                     << "] before softmax: " << attn_score[h * M * M + i * M
  //                     + j] <<
  //                     "\n";
  //         }
  //         Tacc exp_val = std::exp(attn_score[h * M * M + i * M + j] -
  //         max_score); attn_score[h * M * M + i * M + j] = exp_val; if (h == 0
  //         && i == 0 && j < 10) {
  //           std::cout << "attn_scores[" << h << "][" << i << "][" << j
  ...
  //       // Normalize
  //       for (int j = 0; j < M; ++j) {
  //         attn_score[h * M * M + i * M + j] = Tout(attn_score[h * M * M + i *
  //         M + j] / sum_exp); if (h == 0 && i == 0 && j < 10) {
  //           std::cout << "attn_scores[" << h << "][" << i << "][" << j
  //                     << "] after softmax: " << attn_score[h * M * M + i * M
  //                     + j] <<
  //                     "\n";
  //         }
  //       }
  ...
  //   // 4. For each head, calculate attn_score * V_proj and save to
  //   attn_score_v std::vector<Tout> attn_score_v = std::vector<Tout>(M * N,
  //   0); for (int h = 0; h < num_heads; ++h) {
  //     for (int i = 0; i < M; ++i) {
  //       for (int n = 0; n < head_dim; ++n) {
  //         Tacc sum = 0;
  //         for (int j = 0; j < M; ++j) {
  ...
  //           sum += attn_score[h * M * M + attn_idx] * V_proj[v_idx];
  //         }
