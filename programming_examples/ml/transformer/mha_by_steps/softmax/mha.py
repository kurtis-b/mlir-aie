...
def my_mha(
    ...
    def device_body():
        ...
        softmax_l2_ty = np.ndarray[(softmax_dims[0] * softmax_dims[1] * len(l1_fuse_mtx_in[ATTN_SCORE_STR][L1_POS_STR]),), np.dtype[dtype_out]]
        ...
        softmax_l1l2_fifos = [object_fifo(
            f"softmax_L1L2_{l1_pos[ROW_IDX]}_{l1_pos[COL_IDX]}_{l3_fuse_mtx_out[SOFTMAX_STR][L2_POS_STR]}", 
            core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]], 
            mem_tiles[l3_fuse_mtx_out[SOFTMAX_STR][L2_POS_STR]], 
            fifo_depth, 
            softmax_l1_ty,
        ) for l1_pos in l1_fuse_mtx_in[ATTN_SCORE_STR][L1_POS_STR]]
        # L2 to L3 data movement
        softmax_l2l3_fifos = object_fifo(
            f"softmax_L2L3", 
            mem_tiles[l3_fuse_mtx_out[SOFTMAX_STR][L2_POS_STR]], 
            shim_tiles[l3_fuse_mtx_out[SOFTMAX_STR][L3_POS_STR]], 
            fifo_depth, 
            softmax_l2_ty,)
        # L1 to L3 links
        object_fifo_link(softmax_l1l2_fifos, softmax_l2l3_fifos, [softmax_dims[0] * softmax_dims[1] * i for i in range(len(l1_fuse_mtx_in[ATTN_SCORE_STR][L1_POS_STR]))], [])
                ...
                for col_offset in range(0, N, output_mm_dims[2]):
                    for head_offset in range(0, N, head_dim * len(left_mtx_in[Q_STR][L1_POS_STR])):
                        ...
                        npu_dma_memcpy_nd(
                            ...
                            bd_id=2,
                            mem=C,
                            offsets=[0, 0, 0, 3 * M * N + (head_offset // head_dim) * M * M + row_offset * M],
