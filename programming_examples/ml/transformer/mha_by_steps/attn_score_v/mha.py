...
def my_mha(
    ...
    def device_body():
        ...
        attn_score_v_l2_ty = np.ndarray[(attn_score_v_mm_dims[0] * attn_score_v_mm_dims[2]* len(l1_fuse_mtx_in[SOFTMAX_STR][L1_POS_STR]),), np.dtype[dtype_out]]
        ...
        zero_attn_score_v = external_func(
            f"zero_{dtype_out_str}_{attn_score_v_mm_dims[0]}_{attn_score_v_mm_dims[1]}_{attn_score_v_mm_dims[2]}_1", 
            inputs=[attn_score_v_l1_ty]
        )
        matmul_attn_score_v = external_func(
            matmul_vectorized_func_name + f"_{attn_score_v_mm_dims[0]}_{attn_score_v_mm_dims[1]}_{attn_score_v_mm_dims[2]}_1",
        ...
        for l1_src, l1_dst in zip(softmax_l1l2_fifos, softmax_l2l1_fifos):
            object_fifo_link(l1_src, l1_dst)
        # L1 to L2 data movement 
        attn_score_v_l1l2_fifos = [object_fifo(
            f"attn_score_v_L1L2_{l1_pos[ROW_IDX]}_{l1_pos[COL_IDX]}", 
            core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]], 
            mem_tiles[l3_fuse_mtx_out[ATTN_SCORE_V_STR][L2_POS_STR]], 
            fifo_depth, 
            attn_score_v_l1_ty,
        ) for l1_pos in l1_fuse_mtx_in[SOFTMAX_STR][L1_POS_STR]]
        # L2 to L3 data movement
        attn_score_v_l2l3_fifos = object_fifo(
            f"attn_score_v_L2L3", 
            mem_tiles[l3_fuse_mtx_out[ATTN_SCORE_V_STR][L2_POS_STR]], 
            shim_tiles[l3_fuse_mtx_out[ATTN_SCORE_V_STR][L3_POS_STR]], 
            fifo_depth, 
            attn_score_v_l2_ty,
            [
                (attn_score_v_mm_dims[0] // r, r * attn_score_v_mm_dims[2]),
                (r, t),
                (attn_score_v_mm_dims[2] // t, r * t),
                (t, 1),
            ],
        )
        # L1 to L3 links
        object_fifo_link(attn_score_v_l1l2_fifos, attn_score_v_l2l3_fifos, [attn_score_v_mm_dims[0] * attn_score_v_mm_dims[2] * i for i in range(len(l1_fuse_mtx_in[SOFTMAX_STR][L1_POS_STR]))], [])
                ...
                for _ in range_(0xFFFFFFFF):
                    for _ in range_(H // len(left_mtx_in[SOFTMAX_STR][L1_POS_STR]):
                        ...
                        for _ in range_(head_dim // attn_score_v_mm_dims[2]):
                            ...
                            attn_score_v_l1l2_fifos[head].release(ObjectFifoPort.Produce, 1)
