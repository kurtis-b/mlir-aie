...
def my_mha(
    ...
    @device(dev_ty)
    def device_body():
        Xq_l1_ty = np.ndarray[(32 * 192,), np.dtype[dtype_in]]
        Wq_l1_ty = np.ndarray[(192 * 64,), np.dtype[dtype_in]]
        q_l1_ty = np.ndarray[(32 * 64,), np.dtype[dtype_out]]

        Xkv_l1_ty = np.ndarray[(48 * 256,), np.dtype[dtype_in]]
        Wk_l1_ty = np.ndarray[(64 * 48,), np.dtype[dtype_in]]
        k_l1_ty = np.ndarray[(64 * 256,), np.dtype[dtype_out]]

        o1_l1_ty = np.ndarray[(32 * 256,), np.dtype[dtype_out]]
        o2_l1_ty = np.ndarray[(32 * 256,), np.dtype[dtype_out]]

        Wv_l1_ty = np.ndarray[(48 * 64,), np.dtype[dtype_in]]
        v_l1_ty = np.ndarray[(256 * 64,), np.dtype[dtype_out]]

        o3_l1_ty = np.ndarray[(32 * 64,), np.dtype[dtype_out]]

        Wo_l1_ty = np.ndarray[(64 * 192,), np.dtype[dtype_in]]
        o4_l1_ty = np.ndarray[(32 * 192,), np.dtype[dtype_out]]
        Wo_l2_ty = np.ndarray[(64 * 768,), np.dtype[dtype_in]]
        o4_l2_ty = np.ndarray[(32 * 768,), np.dtype[dtype_out]]
        ...
        zero_q = external_func(f"zero_{dtype_out_str}" + f"_{32}_{192}_{64}", inputs=[q_l1_ty])
        zero_k = external_func(f"zero_{dtype_out_str}" + f"_{64}_{48}_{256}", inputs=[k_l1_ty])
        zero_v = external_func(f"zero_{dtype_out_str}" + f"_{256}_{48}_{64}", inputs=[v_l1_ty])
        zero_o1 = external_func(f"zero_{dtype_out_str}" + f"_{32}_{64}_{256}", inputs=[o1_l1_ty])
        zero_o3 = external_func(f"zero_{dtype_out_str}" + f"_{32}_{256}_{64}", inputs=[o3_l1_ty])
        zero_o4 = external_func(f"zero_{dtype_out_str}" + f"_{32}_{64}_{192}", inputs=[o4_l1_ty])
        ...
        matmul_q = external_func(matmul_vectorized_func_name + f"_{32}_{192}_{64}", inputs=[Xq_l1_ty, Wq_l1_ty, q_l1_ty],)
        matmul_k = external_func(matmul_vectorized_func_name + f"_{64}_{48}_{256}", inputs=[Wk_l1_ty, Xkv_l1_ty, k_l1_ty],)
        matmul_v = external_func(matmul_vectorized_func_name + f"_{256}_{48}_{64}", inputs=[Xkv_l1_ty, Wv_l1_ty, v_l1_ty],)
        matmul_o1 = external_func(matmul_vectorized_func_name + f"_{32}_{64}_{256}", inputs=[q_l1_ty, k_l1_ty, o1_l1_ty],)
        softmax_o2 = external_func(f"softmax_{dtype_in_str}", inputs=[o1_l1_ty, o2_l1_ty, np.int32],)
        matmul_o3 = external_func(matmul_vectorized_func_name + f"_{32}_{256}_{64}", inputs=[o2_l1_ty, v_l1_ty, o3_l1_ty],)
        matmul_o4 = external_func(matmul_vectorized_func_name + f"_{32}_{64}_{192}", inputs=[o3_l1_ty, Wo_l1_ty, o4_l1_ty],)
        ...
        object_fifo_link(Wk_l3l1_fifos, Wk_l1l1_fifos)
        Wv_l3l1_fifos = object_fifo(f"Wv_L3L1", shim_tiles[0], core_tiles[3][0], fifo_depth, Wv_l1_ty)
        Wv_l1l1_fifos = object_fifo(f"Wv_L1L1", core_tiles[3][0], core_tiles[2][0], fifo_depth, Wv_l1_ty)
        object_fifo_link(Wv_l3l1_fifos, Wv_l1l1_fifos)
        q_l1l1_fifos = object_fifo(f"q_L1L1", core_tiles[0][1], core_tiles[1][1], fifo_depth, q_l1_ty)
        k_l1l1_fifos = object_fifo(f"k_L1L1", core_tiles[1][0], core_tiles[1][1], fifo_depth, k_l1_ty)
        v_l1l1_fifos_0 = object_fifo(f"v_L1L1_0", core_tiles[2][0], core_tiles[3][0], fifo_depth, v_l1_ty)
        v_l1l1_fifos_1 = object_fifo(f"v_L1L1_1", core_tiles[3][0], core_tiles[3][1], fifo_depth, v_l1_ty)
        ...
        def core_body():
            for _ in range_(0xFFFFFFFF):
                for _ in range_(H):
                    for _ in range_(head_dim // 64):
                        elem_k = k_l1l1_fifos.acquire(ObjectFifoPort.Produce, 1)
                        # zero_k(elem_k)
                        for _ in range_(K // 48):
                            elem_in_xk = Xkv_l3l1_fifos.acquire(ObjectFifoPort.Consume, 1)
                            ...
                            Xkv_l3l1_fifos.release(ObjectFifoPort.Consume, 1)
        ...
        def core_body():
            for _ in range_(0xFFFFFFFF):
                for _ in range_(H):
                    for _ in range_(head_dim // 64):
                        elem_v = v_l1l1_fifos_0.acquire(ObjectFifoPort.Produce, 1)
                        # zero_v(elem_v)
                        for _ in range_(K // 48):
                            elem_in_xv = Xkv_l3l1_fifos.acquire(ObjectFifoPort.Consume, 1)
                            ...
                            Xkv_l3l1_fifos.release(ObjectFifoPort.Consume, 1)
                        v_l1l1_fifos_0.release(ObjectFifoPort.Produce, 1)
            ...
            for _ in range_(0xFFFFFFFF):
                for _ in range_(H):
                    ...
                    for _ in range_(head_dim // 64):
                        ...
                        # zero_o3(elem_o3)
                        elem_in_v = v_l1l1_fifos_1.acquire(ObjectFifoPort.Consume, 1)
                        matmul_o3(elem_in_o2, elem_in_v, elem_o3)
                        v_l1l1_fifos_1.release(ObjectFifoPort.Consume, 1)
        ...
        def sequence(A, B, C):
                ...
                for row_offset in range(0, M, 32):
                    ...
                    npu_dma_memcpy_nd(
                        ...
                        mem=A,
                        offsets=[0, 0, 0, M * K],
                        sizes=[N // 64, K // 192, 192, 64],
                        strides=[64, 192 * N, N, 1],
                    )

                    npu_dma_memcpy_nd(
                        metadata=Xkv_l3l1_fifos,
                        bd_id=1,
                        mem=A,
                        offsets=[0, 0, 0, 0],
                        sizes=[N // 64, K // 48, M, 48],
                        strides=[0, 48, K, 1],
                    )

                    npu_dma_memcpy_nd(
                        metadata=Wk_l3l1_fifos,
                        bd_id=3,
                        mem=A,
                        offsets=[0, 0, 0, M * K + K * N],
                        sizes=[N // 64, K // 48, 48, 64],
                        strides=[64, 48 * N, N, 1],
                    )

                    # B input transfer:
                    npu_dma_memcpy_nd(
                        metadata=Wv_l3l1_fifos,
                        bd_id=4,
                        mem=B,
                        offsets=[0, 0, 0, 0],
                        sizes=[N // 64, K // 48, 48, 64],
