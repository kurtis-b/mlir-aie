...
def main():
    ...
    argparser.add_argument(
        ...
        default=(1, 1),
        help="A tuple argument, e.g. --trace-tile '(1, 1)'"
    )
    argparser.add_argument(
        "--full-design",
        action="store_true",
        help="Use the full design. This flag is for running trace--a compiler error will pop up if the full design is used to build with trace."
    )
    argparser.add_argument(
        "--generate-taps",
        action="store_true",
...
def my_mha(
    ...
    if trace_tile in left_mtx_in[X_STR][L1_POS_STR] + right_mtx_in[WQ_STR][L1_POS_STR] + right_mtx_in[WK_STR][L1_POS_STR] + right_mtx_in[WV_STR][L1_POS_STR]:
        trace_task = "proj"
    elif trace_tile in left_mtx_in[Q_STR][L1_POS_STR] + right_mtx_in[K_STR][L1_POS_STR] + right_mtx_in[V_STR][L1_POS_STR] + right_mtx_in[WO_STR][L1_POS_STR] + l1_fuse_mtx_in[ATTN_SCORE_STR][L1_POS_STR] + l1_fuse_mtx_in[SOFTMAX_STR][L1_POS_STR] + l1_fuse_mtx_in[ATTN_SCORE_V_STR][L1_POS_STR] + l1_fuse_mtx_in[ACCUM_STR][L1_POS_STR]:
        trace_task = "mha"
    else:
        assert False, f"Invalid trace tile {trace_tile}. Looks like the design might not use this tile, or the check for trace_task needs to be updated."
    ...
    @device(dev_ty)
    def device_body():
        if trace_task == "proj" or full_design:
        ...
        tiles_to_trace = [core_tiles[trace_tile[0]][trace_tile[1]]]
        if enable_tracing:
            trace_utils.configure_packet_tracing_flow(tiles_to_trace, shim_tiles[l3_fuse_mtx_out[TRACE_STR][L3_POS_STR]])
        ...
        def sequence(A, B, C):
            if enable_tracing:
                trace_utils.configure_packet_tracing_aie2(
                    ...
                    shim=shim_tiles[l3_fuse_mtx_out[TRACE_STR][L3_POS_STR]],
            ...
            if enable_tracing:
                trace_utils.gen_trace_done_aie2(shim_tiles[l3_fuse_mtx_out[TRACE_STR][L3_POS_STR]])
