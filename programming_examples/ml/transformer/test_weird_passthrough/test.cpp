...
int main(int argc, const char *argv[]) {
  ...
  {
    bufInA[i] = i;
    if (i < 10 || i > PASSTHROUGH_SIZE - 10)
      std::cout << "Initializing bo_inA[" << i << "] = " << std::to_string(bufInA[i]) << std::endl;
  }
  ...
  for (int i = 0; i < PASSTHROUGH_SIZE; i++) {
    if (i < 10 || i > PASSTHROUGH_SIZE - 10)
      std::cout << "Expected " << std::to_string(bufInA[i])
                << ", got " << std::to_string(bufOut[i]) << std::endl;
