...
int main(int argc, const char *argv[]) {
  ...
  int M = vm["M"].as<int>();
  int K = vm["K"].as<int>();
  bool do_verify_stochastic = (long long)M * K > verify_stochastic_threshold;
  ...
  C_DATATYPE *bufOut = bo_out.map<C_DATATYPE *>();
  std::vector<C_DATATYPE> CVec(C_VOLUME);
  memset(CVec.data(), 0,
         C_VOLUME * sizeof(C_DATATYPE)); // Clear the output buffer
  memcpy(bufOut, CVec.data(), (CVec.size() * sizeof(C_DATATYPE)));
  if (trace_size > 0) {
    memset(((char *)bufOut) + C_SIZE, 0, trace_size); // Clear the trace buffer
  }
  ...
  if (verbosity >= 2) {
    ...
    std::cout << "B = \n";
    matmul_common::print_matrix(BVec, 1);
  }
    ...
    if (do_verify) {
      ...
      if (do_verify_stochastic) {
        errors = matmul_common::verify_stochastic<A_DATATYPE, C_DATATYPE,
                                                  ACC_DATATYPE>(
            M, 1, K, AVec, BVec, CVec, verify_stochastic_n_samples, verbosity,
            abs_tol, rel_tol, b_col_maj);
      } else {
        errors = matmul_common::verify<A_DATATYPE, C_DATATYPE, ACC_DATATYPE>(
            M, 1, K, AVec, BVec, CVec, verbosity, abs_tol, rel_tol, b_col_maj);
      }
