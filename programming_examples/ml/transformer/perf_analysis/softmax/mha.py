...
def my_mha(
    ...
    def device_body():
        ...
        q_l1l2_fifos = [object_fifo(
            ...
            mem_tiles[mtx_out[Q_STR][L2_POS_STR]], 
            fifo_depth, 
            q_l1_ty_out,
        ) for l1_pos in right_mtx_in[WQ_STR][L1_POS_STR]]
        k_l1l2_fifos = [object_fifo(
            ...
            core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]], 
            mem_tiles[mtx_out[K_STR][L2_POS_STR]], 
            fifo_depth, 
            k_l1_ty_out,
        ) for l1_pos in right_mtx_in[WK_STR][L1_POS_STR]]
        v_l1l2_fifos = [object_fifo(
            ...
            core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]], 
            mem_tiles[mtx_out[V_STR][L2_POS_STR]], 
            fifo_depth, 
            v_l1_ty_out,
        ) for l1_pos in right_mtx_in[WV_STR][L1_POS_STR]]
        ...
        q_l2l3_fifos = object_fifo(
            f"q_L2L3", 
            mem_tiles[mtx_out[Q_STR][L2_POS_STR]], 
            shim_tiles[mtx_out[Q_STR][L3_POS_STR]], 
            fifo_depth, 
            q_l2_ty,
            [
        ...
        k_l2l3_fifos = object_fifo(
            f"k_L2L3", 
            mem_tiles[mtx_out[K_STR][L2_POS_STR]], 
            shim_tiles[mtx_out[K_STR][L3_POS_STR]], 
            fifo_depth, 
            k_l2_ty,
            [
        ...
        v_l2l3_fifos = object_fifo(
            f"v_L2L3", 
            mem_tiles[mtx_out[V_STR][L2_POS_STR]], 
            shim_tiles[mtx_out[V_STR][L3_POS_STR]], 
            fifo_depth, 
            v_l2_ty,
            [
        ...
        attn_score_l1l2_fifos = [object_fifo(
            f"attn_score_L1L2_{l1_pos_src[ROW_IDX]}_{l1_pos_src[COL_IDX]}_{l1_pos_dst[ROW_IDX]}_{l1_pos_dst[COL_IDX]}", 
            core_tiles[l1_pos_src[ROW_IDX]][l1_pos_src[COL_IDX]], 
            mem_tiles[l1_pos_dst[COL_IDX]], 
            fifo_depth, 
            attn_score_l1_ty,
        ) for l1_pos_src, l1_pos_dst in zip(left_mtx_in[Q_STR][L1_POS_STR], intermediate_mtx_in[ATTN_SCORE_STR][L1_POS_STR])]
        attn_score_l2l1_fifos = [object_fifo(
            f"attn_score_L2L1_{l1_pos_src[ROW_IDX]}_{l1_pos_src[COL_IDX]}_{l1_pos_dst[ROW_IDX]}_{l1_pos_dst[COL_IDX]}", 
            mem_tiles[l1_pos_dst[COL_IDX]], 
            core_tiles[l1_pos_dst[ROW_IDX]][l1_pos_dst[COL_IDX]], 
            fifo_depth, 
            attn_score_l1_ty,
            [
        ...
        softmax_l1l2_fifos = [object_fifo(
            f"softmax_L1L2_{l1_pos[ROW_IDX]}_{l1_pos[COL_IDX]}", 
            core_tiles[l1_pos[ROW_IDX]][l1_pos[COL_IDX]], 
        ...
        softmax_l2l3_fifos = object_fifo(
            f"attn_score_L2L3", 
            mem_tiles[mtx_out[ATTN_SCORE_STR][L2_POS_STR]], 
            shim_tiles[mtx_out[ATTN_SCORE_STR][L3_POS_STR]], 
            fifo_depth, 
            attn_score_l2_ty,
            [
                (softmax_dims[0] // r, r * softmax_dims[1]),
        ...
        # L1 to L3 links
        object_fifo_link(softmax_l1l2_fifos, softmax_l2l3_fifos, [softmax_dims[0] * softmax_dims[1] * i for i in range(len(intermediate_mtx_in[ATTN_SCORE_STR][L1_POS_STR]))], [])
        ...
        for head, l1_pos in enumerate(intermediate_mtx_in[ATTN_SCORE_STR][L1_POS_STR]):
        ...
        def sequence(A, B, C):
                ...
                for head in range(0, H, len(left_mtx_in[Q_STR][L1_POS_STR])):
                    for row_offset in range(0, M, attn_score_mm_dims[0]):
                        ...
                        npu_dma_memcpy_nd(
                            metadata=softmax_l2l3_fifos,
                            bd_id=2,
                            mem=C,
                            offsets=[0, 0, 0, 3 * M * N + head * M * M + row_offset * M],
                            sizes=[1, len(left_mtx_in[Q_STR][L1_POS_STR]), softmax_dims[0], softmax_dims[1]],
                            strides=[0, M * M, M, 1],
                        )

                        dma_wait(softmax_l2l3_fifos)
