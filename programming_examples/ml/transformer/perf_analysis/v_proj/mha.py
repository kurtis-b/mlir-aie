...
def my_mha(
    ...
    def device_body():
        ...
        Xkv_l2l1_fifos = object_fifo(f"Xkv_L2L1", mem_tiles[0], core_tiles[2][0], fifo_depth, Xkv_l1_ty,
                                    [
        ...
        v_l1l2_fifos = object_fifo(f"v_L1L2", core_tiles[2][0], mem_tiles[0], fifo_depth, kv_l1_ty) 
        v_l2l3_fifos = object_fifo(f"v_L2L3", mem_tiles[0], shim_tiles[0], fifo_depth, kv_l1_ty,
                                    [
        ...
        object_fifo_link(v_l1l2_fifos, v_l2l3_fifos)
            ...
            for _ in range_(0xFFFFFFFF):
                for _ in range_(H):
                    for _ in range_(head_dim // kv_matmul_dims[2]):
                        elem_v = v_l1l2_fifos.acquire(ObjectFifoPort.Produce, 1)
                        zero_kv(elem_v)
                        ...
                        v_l1l2_fifos.release(ObjectFifoPort.Produce, 1)
        ...
        def sequence(A, B, C):
                ...
                for row_offset in range(0, M, o4_matmul_dims[0]):
                    ...
                    npu_dma_memcpy_nd(
                        ...
                        mem=C,
                        offsets=[0, 0, 0, 0],
                        sizes=[1, N // kv_matmul_dims[2], M, kv_matmul_dims[2]],
