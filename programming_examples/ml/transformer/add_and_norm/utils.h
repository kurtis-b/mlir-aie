...
std::int16_t get_random<std::int16_t>() {
  return std::abs((std::int16_t)rand() % 0x100);
}

template <>
int8_t get_random<int8_t>() {
  return std::abs((int8_t)rand() % 0x100);
}
...
template <typename Tin, typename Tout, typename Tacc>
int verify(int M, int N, std::vector<Tin> A, std::vector<Tin> B,
...
template <typename Tin, typename Tout, typename Tacc>
int verify_stochastic(int M, int N, std::vector<Tin> A, std::vector<Tin> B,
  ...
  for (std::tuple<size_t, std::tuple<int &, int &>> cell :
    ...
    Tout ref = addandnorm_acc<Tin, Tout, Tacc>(M, N, row, col, A, B, b_col_maj);
