...
void gemv_vectorized(T_in *__restrict a, T_in *__restrict b,
                     T_out *__restrict c) {
...
#define gemv_scalar_c_func(ctype_in, mlir_type_in, ctype_out, mlir_type_out,   \
                           ctype_acc)                                          \
  void gemv_scalar_##mlir_type_in##_##mlir_type_out(                           \
      ctype_in *a_in, ctype_in *b_in, ctype_out *c_out) {                      \
    gemv_scalar<ctype_in, ctype_out, DIM_M, DIM_K>(a_in, b_in, c_out);         \
  }

#define gemv_vectorized_c_func(ctype_in, mlir_type_in, ctype_out,              \
                               mlir_type_out, ctype_acc)                       \
  void gemv_vectorized_##mlir_type_in##_##mlir_type_out(                       \
      ctype_in *a_in, ctype_in *b_in, ctype_out *c_out) {                      \
    gemv_vectorized<ctype_in, ctype_out, ctype_acc, DIM_M, DIM_K, 16, 8>(      \
        a_in, b_in, c_out);                                                    \
  }
...
combos(gemv_scalar_c_func) combos(gemv_vectorized_c_func)
